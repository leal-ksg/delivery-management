...
export class PurchaseController implements IPurchaseController{
    ...
    async createPurchase(purchase: Omit<Purchase, "id">): Promise<HttpResponse<Purchase>> {
        const result = await this.purchaseRepository.create(purchase)

        return toHttpResponse(result)
    }

    async updatePurchase(id: number, purchase: Partial<Purchase>): Promise<HttpResponse<Purchase>> {
        const result = await this.purchaseRepository.update(id, purchase)

        return toHttpResponse(result)
    }

    async cancelPurchase(id: number): Promise<HttpResponse<void>> {
        const result = await this.purchaseRepository.update(id, {status: 'CANCELLED'})
