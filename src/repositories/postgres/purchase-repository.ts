...
export class PurchaseRepository implements IPurchaseRepository {
    ...
    async create(purchase: Omit<Purchase, "id">): Promise<Result<Purchase>> {
        try {
            const newPurchase = await prisma.purchase.create({data: purchase})
    ...
    async update(id: number, purchase: Partial<Purchase>): Promise<Result<Purchase>> {
        try {
            const updatedPurchase = await prisma.purchase.update({where: { id }, data: purchase})
